...
ipcRenderer.on('start-video-recording', async () => {
    try {
        ...
        console.log(`✓ Multi-screen recording started for ${mediaRecorders.length} displays`);
...
async function setupScreenRecording(source, screenIndex) {
    ...
    // Get actual display resolution for this source
    const displays = await ipcRenderer.invoke('get-display-info');
    const sourceDisplay = displays.find(d => source.id.includes(d.id));
    ...
    const stream = await navigator.mediaDevices.getUserMedia({
        ...
        video: {
            mandatory: {
                ...
                // Use actual display resolution - don't constrain
                minWidth: sourceDisplay ? sourceDisplay.bounds.width : 1920,
                maxWidth: sourceDisplay ? sourceDisplay.bounds.width : 7680,
                minHeight: sourceDisplay ? sourceDisplay.bounds.height : 1080,
                maxHeight: sourceDisplay ? sourceDisplay.bounds.height : 4320,
                maxFrameRate: 120 // Higher frame rate capability
            },
            ideal: {
                frameRate: 120,
                // Match exact display resolution
                width: sourceDisplay ? sourceDisplay.bounds.width : 1920,
                height: sourceDisplay ? sourceDisplay.bounds.height : 1080
            }
        }
    });
    ...
    // Ultra-high quality bitrate calculation:
    // Base rate for lossless quality + scaling for resolution and framerate
    const baseBitrate = 300000000; // 300 Mbps base for 1080p60
    const scaleFactor = (resolution / (1920 * 1080)) * (frameRate / 60);
    const calculatedBitrate = Math.max(baseBitrate * scaleFactor, 200000000); // Minimum 200 Mbps
    ...
    // Try AV1 MP4 (excellent compression)
        ...
        videoBitsPerSecond = calculatedBitrate * 0.8; // AV1 is more efficient
    }
    // Try VP9 WebM (high quality, good compression)
    else if (MediaRecorder.isTypeSupported('video/webm;codecs=vp9')) {
        mimeType = 'video/webm;codecs=vp9';
        videoBitsPerSecond = calculatedBitrate * 0.9; // VP9 is efficient
    }
    // Fall back to VP8 WebM
    else if (MediaRecorder.isTypeSupported('video/webm;codecs=vp8')) {
        mimeType = 'video/webm;codecs=vp8';
        videoBitsPerSecond = calculatedBitrate; // VP8 needs higher bitrate
    }
    // Generic fallbacks
    ...
    // Enhanced MediaRecorder options for maximum quality
    const mediaRecorderOptions = {
        mimeType: mimeType,
        videoBitsPerSecond: videoBitsPerSecond,
        bitsPerSecond: videoBitsPerSecond * 1.2, // Higher overall bitrate allowance
        audioContext: null // Disable audio processing overhead
    };

    // Add codec-specific quality parameters if supported
    if (mimeType.includes('h264')) {
        mediaRecorderOptions.videoKeyFrameIntervalDuration = 1000; // 1 second keyframes
    }
    ...
    mediaRecorder.start(100); // Record in 0.1-second chunks for maximum quality
